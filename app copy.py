...
def get_location():
    # Panggil API ipinfo.io untuk mendapatkan informasi lokasi
    response = requests.get("https://ipinfo.io/")
    data = response.json()
    # Ambil informasi kota, wilayah, dan negara
...
def check_existing_document(document_hash):
    # Cari dokumen dengan hash yang sama di koleksi 'documents'
    existing_document = blockval_mongodb.find_data('documents', {'document_hash': document_hash})
...
def attach_qrcode_to_pdf(document):
    ...
    try:
        ...
        # rect = fitz.Rect(450, 50, 550, 150)  # Posisi QR Code (kanan bawah)
        rect = fitz.Rect(50, 50, 150, 150)  # Posisi QR Code (kiri atas halaman pertama)
...
#=====================================================================================================================================
# def broadcast_new_pow_block(pow_block):
#     """
#     Mengirim hasil mining PoW ke seluruh node untuk diperiksa.
#     Jika gagal, coba lagi beberapa kali sebelum menyerah.
#     """
...
#         try:
#             response = requests.post(f"{node}/validate_pow_block", json={"block": pow_block})
#             if response.status_code == 200:
#                 print(f"✅ Hasil mining dikirim ke {node}")
#             else:
...
def start_mining_pow():
    ...
    if last_poa_block:
        ...
        if is_valid_ledger('pow'):
            ...
            return
        # Setelah berhasil disimpan ke ledger:
        # mining_in_progress = False

        # print(f"✅ Blok PoW baru ditemukan! Mengirim hasil mining ke seluruh node untuk validasi...")

        # # 🔄 **Broadcast hasil mining ke seluruh node**
        # broadcast_new_pow_block(new_pow_block)
...
# @app.route('/validate_pow_block', methods=['POST'])
# def validate_pow_block():
#     """
#     Memeriksa hasil mining dari node lain dan menambahkan ke ledger PoW jika valid.
#     """
#     data = request.get_json()
#     pow_block = data.get("block", [])
#     previous_hash = pow_block[2]
#     poa_hash = pow_block[3]

#     if not pow_block or len(pow_block) < 26:
#         return jsonify({"message": "Blok PoW tidak valid"}), 400

#     # 🔍 **Periksa apakah hash valid**
#     block_data = pow_block[0:1] + pow_block[2:4] + pow_block[5:]  # Ambil semua kecuali hash (idx 1) dan nonce (idx 4)
#     data_string = "".join(str(item) for item in block_data) + str(pow_block[4])
#     recalculated_hash = hashlib.sha256(data_string.encode()).hexdigest()
#     if recalculated_hash != pow_block[1] or not recalculated_hash.startswith("0000"):
#         print(f"❌ ERROR: Blok PoW #{pow_block[0]} memiliki hash tidak valid.")
#         return jsonify({"message": "Blok PoW tidak valid"}), 400
#     print(f"✅ Blok PoW #{pow_block[0]} memiliki hash yang valid.")

#      # 🔍 **Ambil blok terakhir dari ledger_pow**
#     last_pow_block = ledger_pow.fetch_last("ledger_pow")
#     last_block_number = int(last_pow_block[0]) if last_pow_block else 0
#     current_block_number = int(pow_block[0])

#     # # 🚨 **Pastikan hanya menambahkan blok jika lebih baru**
#     # if current_block_number <= last_block_number:
#     #     print(f"⚠️ Blok PoW #{current_block_number} sudah ada atau lebih lama dari blok terakhir. Mengabaikan.")
#     #     return jsonify({"message": "Blok PoW sudah ada atau lebih lama"}), 400
    
#     # 🚨 Tolak jika sudah ada blok dengan number yang sama
#     existing_block = ledger_pow.fetch_one("ledger_pow", f"number = '{current_block_number}'")
#     if existing_block:
#         print(f"⚠️ Blok PoW #{current_block_number} sudah ada, menolak blok dari miner {node_address}")
#         return jsonify({"message": "Block with same number already exists"}), 400
    
#     # 🚨 **Cek apakah previous_hash cocok dengan hash blok terakhir**
#     if last_pow_block and previous_hash != last_pow_block[1]:
#         print(f"❌ ERROR: Previous hash tidak cocok! Blok #{current_block_number} ditolak.")
#         return jsonify({"message": "Previous hash tidak cocok"}), 400
    
#     # **🔍 Validasi PoA Hash dengan Ledger PoA**
#     corresponding_poa_block = ledger_poa.fetch_one("ledger_poa", f"number = '{pow_block[0]}'")
#     if corresponding_poa_block:
#         correct_poa_hash = corresponding_poa_block[1]  # Hash dari ledger PoA
#         if poa_hash != correct_poa_hash:
#             print(f"❌ ERROR: PoA Hash tidak cocok! Blok #{current_block_number} ditolak.")
#             return jsonify({"message": "PoA Hash tidak cocok"}), 400
#         print(f"✅ PoA Hash valid untuk blok #{pow_block[0]}")
    
#     # 🚨 **Cek apakah hash blok sudah ada di ledger untuk menghindari duplikasi**
#     last_block_hash = last_pow_block[1] if last_pow_block else None
#     if last_block_hash and last_block_hash == pow_block[1]:
#         print(f"⚠️ Blok PoW dengan hash {pow_block[1]} sudah ada. Mengabaikan.")
#         return jsonify({"message": "Blok PoW duplikat"}), 400

#     # ✅ **Tambahkan ke ledger jika belum ada**
#     ledger_pow.insert_record("ledger_pow", pow_block)
#     print(f"✅ Blok PoW #{current_block_number} diterima dan ditambahkan ke ledger!")

#     return jsonify({"message": "Blok PoW diterima"}), 200
...
def document_upload_process():
    ...
    if document:
        ...
        existing_document = check_existing_document(document_hash)
        if existing_document:
            if existing_document.get('isValid') == 'true':
                flash("Dokumen sudah pernah diajukan validasi dengan hasil VALID.", "info")
                return redirect(url_for('upload_document'))
            elif existing_document.get('isValid') == 'false':
                flash("Dokumen sudah pernah diajukan validasi dengan hasil TIDAK VALID.", "warning")
                return redirect(url_for('upload_document'))
            elif existing_document.get('isValid') == 'none':
    ...
    try:
        ...
        if document['ki_token'] != '' and document['kompetensi_token'] != '' and document['balai_token'] != '':
            if document['ki_token'] != 'invalid' and document['kompetensi_token'] != 'invalid' and document['balai_token'] != 'invalid':
                ...
                if is_valid_ledger('poa'):  # 🔹 Validasi ledger sebelum menambah blok baru
                    if proof_of_authority(document):
                        flash("Dokumen berhasil divalidasi dan masuk ke blockchain (ledger_poa).", "success")
                        # 🔹 Tempelkan QR Code ke PDF di GridFS

                        error = attach_qrcode_to_pdf(document)

                        if error:
                            flash(f"Gagal menambahkan QR Code: {error}", "danger")
                            return redirect(url_for('validasi_dokumen'))

                        flash("Dokumen berhasil divalidasi, masuk ke ledger PoA, dan QR Code ditambahkan.", "success")
                        return redirect(url_for('validasi_dokumen'))
                                        
                else:
                    flash("Ledger POA tidak valid, blok tidak ditambahkan!", "danger")

            elif document['ki_token'] == 'invalid' or document['kompetensi_token'] == 'invalid' or document['balai_token'] == 'invalid':
